...
export function exportAsOpenAIInstructions(db: Db): string {
  const user = db.select().from(users).where(eq(users.id, LOCAL_USER_ID)).get()
  if (!user) throw new Error('User not found')
  ...
  lines.push('')
  lines.push(`Name: ${summary.userName}`)
  if (summary.occupation) lines.push(`Occupation: ${summary.occupation}`)
  if (summary.location) lines.push(`Location: ${summary.location}`)
  lines.push('')

  if (summary.sections.personalPortrait.content.length > 0) {
    lines.push(`About me:`)
    for (const item of summary.sections.personalPortrait.content.slice(0, 15)) {
      lines.push(`- ${item}`)
    }
    lines.push('')
  }

  if (summary.sections.communicationStyle.content.length > 0) {
    lines.push(`How I communicate:`)
    for (const item of summary.sections.communicationStyle.content.slice(0, 10)) {
      lines.push(`- ${item}`)
    }
    lines.push('')
  }

  if (summary.sections.strengthsAndExpertise.content.length > 0) {
    lines.push(`My strengths and expertise:`)
    for (const item of summary.sections.strengthsAndExpertise.content.slice(0, 10)) {
      lines.push(`- ${item}`)
    }
    lines.push('')
  }
  ...
  if (personalityData.hasAssessment && personalityData.domainScores.length > 0) {
    ...
    for (const ds of personalityData.domainScores) {
      lines.push(`- ${ds.domainLabel}: ${ds.score.toFixed(1)}/5 (${ds.level})`)
    }
    lines.push('')
  }
  ...
  if (summary.sections.toneOfVoice.content.length > 0) {
    lines.push(`Match my communication style:`)
    for (const item of summary.sections.toneOfVoice.content.slice(0, 8)) {
      lines.push(`- ${item}`)
    }
    lines.push('')
  }

  if (summary.sections.decisionMakingPatterns.content.length > 0) {
    lines.push(`Consider how I make decisions:`)
    for (const item of summary.sections.decisionMakingPatterns.content.slice(0, 8)) {
      lines.push(`- ${item}`)
    }
    lines.push('')
  }
...
export function exportAsClaudeMd(db: Db): string {
  const user = db.select().from(users).where(eq(users.id, LOCAL_USER_ID)).get()
  if (!user) throw new Error('User not found')
  ...
  const lines: string[] = []
  lines.push(`# User Context — ${summary.userName}`)
  lines.push('')
  lines.push(`> This file was auto-generated by me.md from ${summary.totalVerifiedInsights} verified insights.`)
  lines.push('')
  lines.push(`## Who I Am`)
  lines.push('')
  if (summary.occupation) lines.push(`- **Role:** ${summary.occupation}`)
  if (summary.location) lines.push(`- **Location:** ${summary.location}`)
  lines.push('')

  const renderSection = (title: string, section: ProfileSection) => {
  ...
  renderSection('Personal Portrait', summary.sections.personalPortrait)
  renderSection('Communication Style', summary.sections.communicationStyle)
  renderSection('Decision-Making Patterns', summary.sections.decisionMakingPatterns)
  renderSection('Strengths & Expertise', summary.sections.strengthsAndExpertise)
  renderSection('Tone of Voice', summary.sections.toneOfVoice)
  ...
  if (summary.sections.keyThemes.content.length > 0) {
    lines.push(`## Key Themes`)
    lines.push('')
    for (const item of summary.sections.keyThemes.content) lines.push(`- ${item}`)
    lines.push('')
  }
...
export function exportAsCursorRules(db: Db): string {
  const user = db.select().from(users).where(eq(users.id, LOCAL_USER_ID)).get()
  if (!user) throw new Error('User not found')
  ...
  lines.push('')
  lines.push(`The developer using this project is ${summary.userName}.`)
  if (summary.occupation) lines.push(`They work as: ${summary.occupation}.`)
  if (summary.location) lines.push(`Based in: ${summary.location}.`)
  lines.push('')
  lines.push(`## Verified Personal Context`)
  lines.push('')
  lines.push(`This context was generated from ${summary.totalVerifiedInsights} verified insights across ${summary.topicsExplored} topics using me.md.`)
  lines.push('')

  if (summary.sections.strengthsAndExpertise.content.length > 0) {
    lines.push(`## Strengths & Expertise`)
    lines.push('')
    for (const item of summary.sections.strengthsAndExpertise.content) lines.push(`- ${item}`)
    lines.push('')
  }
  ...
  if (summary.sections.communicationStyle.content.length > 0) {
    ...
    lines.push('')
    for (const item of summary.sections.communicationStyle.content) lines.push(`- ${item}`)
    lines.push('')
  }
  ...
  if (summary.sections.decisionMakingPatterns.content.length > 0) {
    ...
    lines.push('')
    for (const item of summary.sections.decisionMakingPatterns.content) lines.push(`- ${item}`)
    lines.push('')
  }
