...
import { topics, sessions, insights, notes, verificationHistory, bookmarks } from '@/db/schema'
...
/**
 * Get dashboard statistics for the local user.
 */
export function getDashboardStats(db: Db) {
  ...
  const exploredTopicIds = new Set(userSessions.map((s: any) => s.topicId))
  ...
  const verifiedInsights = userInsights.filter((i: any) => i.verificationStatus === 'verified').length
  const rejectedInsights = userInsights.filter((i: any) => i.verificationStatus === 'rejected').length
  ...
  const categoryCompleteness = KNOWLEDGE_CATEGORIES.map(category => {
    const categoryTopics = userTopics.filter((t: any) => t.presetCategory === category)
    const totalInCategory = categoryTopics.length
    const exploredInCategory = categoryTopics.filter((t: any) => exploredTopicIds.has(t.id)).length
    const categoryTopicIds = new Set(categoryTopics.map((t: any) => t.id))
    const categoryInsights = userInsights.filter((i: any) => categoryTopicIds.has(i.topicId))
    const verifiedInCategory = categoryInsights.filter((i: any) => i.verificationStatus === 'verified').length
  ...
  const topicInsightBreakdown = userTopics.map((topic: any) => {
    const topicInsightsList = userInsights.filter((i: any) => i.topicId === topic.id)
    const verified = topicInsightsList.filter((i: any) => i.verificationStatus === 'verified').length
    const rejected = topicInsightsList.filter((i: any) => i.verificationStatus === 'rejected').length
    const unverified = topicInsightsList.filter((i: any) => i.verificationStatus === 'unverified' || i.verificationStatus === 're_verification_pending').length
    return {
      topicId: topic.id,
  ...
  }).filter((t: any) => t.totalInsights > 0)
    .sort((a: any, b: any) => b.totalInsights - a.totalInsights)
...
/**
 * Get recent activity for the local user (aggregated from multiple sources).
 */
export function getActivityData(db: Db) {
  const userId = LOCAL_USER_ID
  ...
  return { activity: allActivities.slice(0, 20) }
}
