 ...
 * me.md MCP Server
 *
 * A standalone Model Context Protocol server that exposes personal knowledge
 ...
 *   npx tsx mcp-server.ts --db-path /path/to/memd.db
 *
 * Default database path: ~/.memd/memd.db
 *
 ...
 *     "me-md": {
 *       "command": "npx",
 *       "args": ["tsx", "/path/to/mcp/mcp-server.ts"]
 *     }
 *   }
 * }
 */
...
function parseArgs(): { dbPath: string } {
  ...
  if (!dbPath) {
    dbPath = path.join(os.homedir(), '.memd', 'memd.db');
  }
...
if (!fs.existsSync(dbPath)) {
  ...
  process.stderr.write('Export your database from me.md first (Settings > Export for MCP).\n');
  process.stderr.write('Or specify a custom path: npx tsx mcp-server.ts --db-path /path/to/memd.db\n');
  process.exit(1);
}
...
// In local-only mode, there is a single user. Look up the first user.
const userRow = sqlite.prepare('SELECT id, name, email, occupation, location FROM users LIMIT 1').get() as {
  id: string;
  name: string;
  email: string;
  occupation: string;
...
if (!userRow) {
  process.stderr.write('Error: No user found in database. Use me.md to create your profile first.\n');
  process.exit(1);
}

const userId = userRow.id;

process.stderr.write(`[me.md MCP] Starting MCP server for user: ${userRow.name || 'Unknown'}\n`);
process.stderr.write(`[me.md MCP] Database: ${dbPath}\n`);
...
server.resource(
  ...
  'user://profile',
  {
    description: `Verified personal profile and insights for ${userRow.name || 'Unknown'}`,
...
server.resource(
  ...
  'user://personality',
  {
    description: `Big Five personality assessment data for ${userRow.name || 'Unknown'}`,
    mimeType: 'application/json',
  },
  async (uri) => {
    const personalityData = getPersonalityData();
...
server.tool(
  ...
  async () => {
    ...
    const lines: string[] = [];
    lines.push(`# ${userRow.name || 'Unknown'}'s Personal Context`);
    lines.push('');
